...
public class DataBaseSellCarRepository implements Repository<Car> {
    private final Connection connection;
    public DataBaseSellCarRepository(DataBaseConnector connector){this.connection = connector.getConnection();}

    @Override
    public Car find(Long id) {
        String query = "select * from sellcar where id = ?";
        try (
    ...
    public void addCar(Car source, Long id) {
        String query = "insert into sellcar (model, marka, IDBayer) values (?, ?, ?);";
    ...
    public void remove(Car source) {
        String query = "delete from sellcar where model=? and marka=?;";
        String model = source.getMod()+"";
        String marka = source.getMar()+"";
        try (
                PreparedStatement statement = connection.prepareStatement(query);
